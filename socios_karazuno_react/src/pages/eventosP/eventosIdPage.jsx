          ...
          {showManage && (
            <div className="mt-4 p-4 border rounded-lg bg-gray-50 animate-fade-in">
              ...
              <ul className="space-y-2 list-disc list-inside">
                <li><strong>Inscripción:</strong> ${evento.costo || '0.00'} - Pagar a: <strong>{findUserName(evento.pago_inscripcion_a)}</strong></li>
                <li><strong>Transporte:</strong> ${evento.costo_viaje || '0.00'} - Pagar a: <strong>{findUserName(evento.pago_transporte_a)}</strong></li>
                <li><strong>Hospedaje:</strong> ${evento.costo_hospedaje || '0.00'} - Pagar a: <strong>{findUserName(evento.pago_hospedaje_a)}</strong></li>
                <li><strong>Comida:</strong> ${evento.costo_comida || '0.00'} - Pagar a: <strong>{findUserName(evento.pago_comida_a)}</strong></li>
              </ul>
              
              <h2 className="text-xl font-bold mt-5 mb-3 text-gray-800">Profesores a Cargo</h2>
              <ul className="space-y-1 list-disc list-inside">
                {evento.profesores_a_cargo?.length > 0 ? (
                    evento.profesores_a_cargo.map(profId => (
                        <li key={profId}>{findUserName(profId)}</li>
                    ))
                ) : (
